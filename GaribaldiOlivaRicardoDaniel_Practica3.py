...
archivo_csv = '.\Practica1\concentlite.csv'
...
#print(y)
sets = setsEntrenamiento(1, 0.8, datosNp.copy())
...
# Input data (X)
x = np.delete(sets[0][0], 2, 1)
# Etiquetas (Y)
y = np.array(sets[0][0].T[2],ndmin=2).T

#Entrenando
print("-----Entrenando--------")
red = PMC(input_size=2, hidden_size=4, output_size=1, learning_rate=0.2)
...
# Input data (X)
x_t = np.delete(sets[0][1], 2, 1)
# Etiquetas (Y)
y_t = np.array(sets[0][1].T[2],ndmin=2).T
